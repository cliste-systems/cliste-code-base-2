...
| **Production (LiveKit Cloud) call** | Run `npm run watch:call-transcripts` while testing — polls Supabase and writes only when Caller lines are present |
...
`export:latest-call` reads **Supabase only** (not LiveKit logs). It refuses to overwrite `latest.md` when the latest row is missing Caller lines. Use `--wait` after a phone test; use `--allow-partial` only if you explicitly want a flagged incomplete file.
