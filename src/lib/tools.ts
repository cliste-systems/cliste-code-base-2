...
import { getSalonServices } from './supabase.js';
...
export class SalonTools {
  ...
  readonly bookAppointment = llm.tool({
    ...
    execute: async ({ name, phone, service, datetime }, { ctx }) => {
      const ud = readSalonUserData(ctx);
      ...
      const smsBody = bookingConfirmationSmsBody({
        customerName: name,
      ...
      return {
        ...
        spokenTimeLocal,
        message: smsOk
          ? `Booking saved (ref ${bookingReference}). Confirmation text sent to ${customerPhone}. Say the time using **spokenTimeLocal** (not raw ISO). Repeat the reference aloud. When they need nothing else, invoke endPhoneCall in the same turn as goodbye—do not say "end phone call" as speech.`
          : `Booking saved (ref ${bookingReference}). SMS did not go through (${smsDetail}). Say the time using **spokenTimeLocal**. Read the reference aloud. For hang-up, invoke endPhoneCall—do not say it as words.`,
      };
    },
  });
